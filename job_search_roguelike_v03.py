#!/usr/bin/env python3
# Job Search Roguelike — v0.3
# Adds: Rent cycle, unemployment benefits, resilience, tuning tweaks.
...
class Player:
    ...
    def status(self): return f"Week {self.week} | E{self.energy} | ${self.money} | Conf {self.confidence} | Res {self.resilience:.1f}"
...
def intro()->Player:
    print("=== Job Search Roguelike — v0.3 ===")
...
def act_apply(p:Player):
    ...
    if random.random()<cb:
        ...
        if random.random()<offer:
            p.win_reason=f"Offer in {p.target_industry}! (v0.3)"; p.game_over=True
        else:
            rejection(p)
    else:
        rejection(p)
...
def check_end(p:Player):
    ...
    if p.week>WEEKS_TO_SURVIVE and not p.game_over:
        p.game_over=True; p.win_reason=f"Survival: lasted {WEEKS_TO_SURVIVE} weeks."
    if p.game_over:
